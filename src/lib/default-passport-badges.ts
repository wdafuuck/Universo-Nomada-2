/**
 * Catálogo inicial solo para BD vacía.
 * No se re-aplican al sincronizar: las insignias las administra el admin.
 * Matching multi-destino (ej. "Rio … e Iguazu") usa nombre/destino/matchTerms
 * de las insignias existentes — no hace falta crear una por cada ciudad.
 */
export const DEFAULT_PASSPORT_BADGES = [
  {
    slug: "rapa-nui",
    name: "Isla de Pascua",
    destination: "Rapa Nui",
    description: "Has explorado el ombligo del mundo",
    image: "",
    emoji: "🗿",
    matchTerms: JSON.stringify([
      "rapa nui",
      "rapa-nui",
      "group-rapa-nui",
      "isla de pascua",
      "easter island",
    ]),
    sortOrder: 1,
  },
  {
    slug: "atacama",
    name: "Desierto de Atacama",
    destination: "Atacama",
    description: "Aventura bajo las estrellas del desierto más árido",
    image: "",
    emoji: "🏜️",
    matchTerms: JSON.stringify([
      "atacama",
      "san pedro",
      "san-pedro-uyuni",
      "group-atacama",
      "group-uyuni",
      "region-atacama",
      "uyuni",
    ]),
    sortOrder: 2,
  },
  {
    slug: "patagonia",
    name: "Patagonia",
    destination: "Patagonia",
    description: "Glaciares y naturaleza extrema",
    image: "",
    emoji: "🏔️",
    matchTerms: JSON.stringify([
      "patagonia",
      "torres del paine",
      "punta arenas",
      "catedrales-marmol",
      "catedrales",
    ]),
    sortOrder: 3,
  },
  {
    slug: "machu-picchu",
    name: "Machu Picchu",
    destination: "Perú",
    description: "La ciudadela inca te esperó",
    image: "",
    emoji: "⛰️",
    matchTerms: JSON.stringify([
      "machu picchu",
      "machu-picchu",
      "cusco-machupicchu",
      "cusco",
      "peru",
      "perú",
    ]),
    sortOrder: 4,
  },
  {
    slug: "elqui",
    name: "Valle de Elqui",
    destination: "Elqui",
    description: "Cielos estrellados y pisco",
    image: "",
    emoji: "✨",
    matchTerms: JSON.stringify([
      "elqui",
      "ballenas-elqui",
      "la serena",
      "coquimbo",
      "vicuna",
    ]),
    sortOrder: 5,
  },
];
